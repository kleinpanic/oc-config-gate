 ...
 */
function findRawKeyPaths(obj: unknown, prefix = ""): string[] {
  const paths: string[] = [];
  if (!isObj(obj)) return paths;

  for (const [k, v] of Object.entries(obj)) {
    const path = prefix ? `${prefix}.${k}` : k;
    const lk = k.toLowerCase();
    if (
      (lk === "apikey" || lk === "api_key" || lk === "token" || lk === "secret") &&
      typeof v === "string" &&
      !v.startsWith("${") &&
      RAW_KEY_RE.test(v)
    ) {
...
export function validateMergedConfig(
  ...
  const REQUIRED_SECTIONS = ["agents", "plugins"] as const;
  ...
  if (removedAgents.length > 0) {
    errors.push(
      `agents.list removed ${removedAgents.length} agent(s): ${removedAgents.join(", ")}. ` +
  ...
  if (removedPlugins.length > 0) {
    errors.push(
      `plugins allowlist removed ${removedPlugins.length} plugin(s): ${removedPlugins.join(", ")}`,
    );
  }
  ...
  for (const p of rawKeyPaths) {
    errors.push(
      `Raw API key detected at "${p}". Use \${ENV_VAR_NAME} template syntax instead.`,
    );
  }
  ...
  if (beforeAliasCount > 0 && afterAliasCount < beforeAliasCount) {
    ...
    errors.push(
      `models.providers alias fields dropped from ${beforeAliasCount} to ${afterAliasCount} (-${lost}). ` +
...
function getAgentIds(cfg: unknown): string[] {
  if (!isObj(cfg)) return [];
  const agents = (cfg as JsonObject)["agents"];
  if (!isObj(agents)) return [];
  const list = (agents as JsonObject)["list"];
  if (!Array.isArray(list)) return [];
  return list
    .filter((a): a is JsonObject => isObj(a) && typeof a["id"] === "string")
    .map((a) => a["id"] as string);
}

function getAgentList(cfg: unknown): unknown[] {
  if (!isObj(cfg)) return [];
  const agents = (cfg as JsonObject)["agents"];
  if (!isObj(agents)) return [];
  const list = (agents as JsonObject)["list"];
...
function getPluginAllowlist(cfg: unknown): string[] {
  if (!isObj(cfg)) return [];
  const plugins = (cfg as JsonObject)["plugins"];
  if (!isObj(plugins)) return [];
  const allow = (plugins as JsonObject)["allow"];
  if (!Array.isArray(allow)) return [];
  return allow.filter((p): p is string => typeof p === "string");
}

function countModelAliases(cfg: unknown): number {
  if (!isObj(cfg)) return 0;
  const models = (cfg as JsonObject)["models"];
  if (!isObj(models)) return 0;
  const providers = (models as JsonObject)["providers"];
  if (!isObj(providers)) return 0;

  let count = 0;
  for (const provider of Object.values(providers)) {
    if (!isObj(provider)) continue;
    const modelList = (provider as JsonObject)["models"];
    if (!Array.isArray(modelList)) continue;
    for (const m of modelList) {
      if (isObj(m) && "alias" in m) count++;
    }
  }
  return count;
}
