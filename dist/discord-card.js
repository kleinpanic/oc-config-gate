...
function summarizeDiffStats(diffLines) {
    let added = 0;
    let removed = 0;
    let modified = 0;
...
function diffPreview(diffLines, limit) {
 ...
 */
export function buildApprovalCardComponents(params, allowedUsers) {
...
export function buildApprovalCardText(params) {
    ...
    if (validation.ok) {
        lines.push("```diff", `${accent} stage checks passed`, `${accent} approval will recheck base hash`, `${accent} approval will safety-check, write, bless, and audit`, ...(restart ? [`${accent} approval will request an official safe gateway restart`] : []), "```");
    }
    else {
    ...
    lines.push("", "**Change Summary**", "```text", `changed  ${String(stats.total).padStart(3)}`, `added    ${String(stats.added).padStart(3)}`, `modified ${String(stats.modified).padStart(3)}`, `removed  ${String(stats.removed).padStart(3)}`, "```", "", "**Diff Preview**", "```diff", ...diffPreview(diffLines, 18), "```", "**Approval Behavior**", validation.ok
        ? restart
...
//# sourceMappingURL=discord-card.js.map