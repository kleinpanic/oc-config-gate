...
import { generateRequestId, writePending, readPending, clearPending, isPendingExpired, hashConfig, } from "./pending.js";
...
// ─── oc_config_stage ──────────────────────────────────────────────────────────
export function createConfigStageTool(api, pluginCfg) {
    return {
        ...
        parameters: {
            ...
            properties: {
                patch: {
                    type: "object",
                    description: "JSON Merge Patch (RFC 7396). Only include keys you want to change. Null deletes a key.",
                },
                reason: {
                    type: "string",
        ...
        execute: async (_id, params) => {
            ...
            // Apply merge patch
            const mergedConfig = applyMergePatch(liveConfigRaw, patch);
            // Validate
...
// ─── oc_config_apply ──────────────────────────────────────────────────────────
export function createConfigApplyTool(_api) {
    return {
        ...
        execute: async (_id, params) => {
            ...
            if (reqId && pending.requestId !== reqId) {
                return text(`Pending request is \`${pending.requestId}\`, not \`${reqId}\`. Only one request can be pending at a time.`, true);
            }
...
export async function doApply(pending, api) {
    if (!pending)
        return text("No pending request", true);
    const runtime = api.runtime;
    ...
    if (!finalValidation.ok) {
        ...
        return text(`❌ Final validation failed — config not applied.\nErrors: ${errs}\n\nFix the errors and stage a new request.`, true);
    }
    ...
    try {
        ...
        return text(`Failed to write config: ${err instanceof Error ? err.message : String(err)}`, true);
    }
    ...
    if (pending.restart) {
        ...
        if (restart.ok) {
            ...
            if (restart.output)
                resultLines.push(`Restart output: ${restart.output.slice(0, 500)}`);
        }
        else {
